...
function gbkToUtf8(gbkText, currentEncoding = 'utf8') {
    if (currentEncoding.toLowerCase() === 'utf8' || currentEncoding.toLowerCase() === 'utf-8') return gbkText;
    if (!window['utools']) return gbkText;
    return window.utils.decode(gbkText);
}
export async function parseByUrl(url, config) {
    if (!/(^http(s?):\/\/[^\s$.?#].[^\s]*)/i.test(url)) return null;

    const { data } = await axios(url, config);

    return this.parseByContent(data);
}
export async function parseByContent(data) {
